...
import transpileDynamicModuleLoader from '../transpilers/transpileImports';
 ...
 * - Not intended for generic use — this wrapper is opinionated for single-spa architecture
...
export default function defineConfig(userConfig: UserConfigExport): UserConfigExport {
  ...
  const generateConfig = (config: UserConfig): UserConfig => {
    ...
    const builtConfig = viteDefineConfig({
      ...
      plugins: [...(config.plugins || []), transpileDynamicModuleLoader()],
    });
