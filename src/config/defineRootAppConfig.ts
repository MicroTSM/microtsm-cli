...
export interface MicroTSMRootAppBuildConfig extends UserConfig {
  ...
  /**
   * Enable the static file name hashing on built files to ensure the cache cleared on every build.
   * @default true
   */
...
export default function defineRootAppConfig(config: MicroTSMRootAppBuildConfig) {
  ...
  config.enableFileNameHashing ??= true;
          ...
          typeof buildInput === 'string'
            ...
            : Array.isArray(buildInput)
              ...
              : typeof buildInput === 'object'
                ...
                : config.entryScript,
        output: {
          entryFileNames: () => (config.enableFileNameHashing ? 'js/[name]-[hash].js' : 'js/[name].js'),
          chunkFileNames: () => (config.enableFileNameHashing ? 'js/[name]-[hash].js' : 'js/[name].js'),
