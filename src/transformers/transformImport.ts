...
export function transformImports(code: string) {
  ...
  // Returns true if the moduleSource does NOT start with "." or "/".
  ...
  // The regexp now allows for zero or more spaces between the key parts.
  // It handles cases like:
  //   import { a, b as c } from 'mod';
  //   import d from 'mod';
  //   import * as ns from 'mod';
  //   import{inject,onUnmounted,onDeactivated}from"vue";
  ...
  code = code.replace(importRegex, (match, _clause, moduleSource) => {
    ...
    const assignments = externalImports
      .map((imp, index) => {
        if (imp.clause.startsWith('{')) {
          ...
          const alias = parts[parts.indexOf('as') + 1];
